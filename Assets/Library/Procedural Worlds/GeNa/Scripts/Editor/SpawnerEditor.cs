...
namespace GeNa.Core
{
    ...
    public class GeNaSpawnerPrefabs
    {
        ...
        static void OnPrefabInstanceUpdated(GameObject gameObject)
        {
            ...
            var mainFile = default(GeNaSpawnerData);
    ...
    public class SpawnerEditor : GeNaEditor
    {
        ...
        private Dictionary<int, bool> m_showSpawnCriteriaOverrides = new Dictionary<int, bool>();
        private Dictionary<int, bool> m_showPrototypeAdvanced = new Dictionary<int, bool>();
        private Dictionary<int, bool> m_showPrototypeInEditor = new Dictionary<int, bool>();
        private Dictionary<int, bool> m_showResourceInEditor = new Dictionary<int, bool>();
        private Dictionary<int, bool> m_showChildResourceInEditor = new Dictionary<int, bool>();
        ...
        private void OnEnable()
        {
            ...
            m_placementCriteria = spawnerData.PlacementCriteria;
            m_spawnCriteria = spawnerData.SpawnCriteria;
            m_settings = spawnerData.Settings;
        ...
        {
            var data = GeNaEditorUtility.RevertToMain(spawner);
        ...
        public static void GeNaSpawnerEditor(GeNaSpawner spawner, Action onRepaint = null)
        {
            ...
            SpawnerSettings settings = spawner.SpawnerData.Settings;
            PlacementCriteria placementCriteria = spawner.SpawnerData.PlacementCriteria;
            SpawnCriteria spawnCriteria = spawner.SpawnerData.SpawnCriteria;
            bool raycastHit = GeNaEditorUtility.ShowSpawnRange(spawner.SpawnerData, out RaycastHit hitInfo, m_showVisualizer);
            // If SHIFT is not held down, visualization will be of
            settings.VisualizationActive = false;
            #region Keyboard Handling
            GameObject lastSpawnedObject = spawner.SpawnerData.LastSpawnedObject;
            //Keyboard handling
            ...
            if (e.Equals(GeNaEditorUtility.Defaults.KeyDeleteEvent(false, true)))
            {
                ...
                if (EditorUtility.DisplayDialog("WARNING!",
                    ...
                    "OK", "Cancel"))
                {
                    GeNaSpawnerInternal.DespawnAllPrefabs(spawner.SpawnerData);
                }
                e.Use();
                GUIUtility.hotControl = 0;
                GUIUtility.ExitGUI();
            ...
            if (e.type == EventType.ScrollWheel)
            {
                if (e.control)
                {
                    ...
                    if (offset > 0)
                    {
                        spawner.SpawnerData.MinInstances -= offset;
                        spawner.SpawnerData.MaxInstances -= offset;
                    }
                    else
                    {
                        spawner.SpawnerData.MaxInstances -= offset;
                    ...
                    GeNaSpawnerInternal.UpdateTargetSpawnerRanges(spawner.SpawnerData, false);
                    onRepaint?.Invoke();
                    return;
                }
                if (e.shift)
                {
                    ...
                    var spawnRange = spawner.SpawnerData.SpawnRange;
                    if (spawner.SpawnerData.UseLargeRanges)
                        spawnRange -= scroll;
                    else
                        spawnRange = Mathf.Clamp(spawnRange - scroll, 0, 200);
                    spawner.SpawnerData.SpawnRange = spawnRange;
                    //Settings changed, let's update ranges - probably no need to update child spawners, since their settings did not change.
                    ...
                    GeNaSpawnerInternal.SetSpawnOrigin(spawner.SpawnerData, spawnCall);
                    //Let's update ranges - including child spawners.
                    GeNaSpawnerInternal.UpdateTargetSpawnerRanges(spawner.SpawnerData, false);
                    GeNaSpawnerInternal.UpdateVisualization(spawner.SpawnerData);
                    onRepaint?.Invoke();
                    e.Use();
                    return;
                }
                    ...
                    switch (e.type)
                    {
                        case EventType.MouseDown:
                            ...
                            if (raycastHit)
                            {
                                ...
                                spawner.SpawnerData.SpawnCalls.Clear();
                                SpawnCall spawnCall = GeNaSpawnerInternal.GenerateSpawnCall(spawner.SpawnerData, hitInfo);
                                bool singleSpawnMode = spawner.SpawnerData.SpawnMode != Constants.SpawnMode.Paint;
                                SpawnerEntry entry = new SpawnerEntry(spawner)
                                {
                                    Initialize = true,
                                    Title = "GeNa Spawner",
                                    Info = spawner.SpawnerData.Name + " is Spawning...",
                                    RecordUndo = singleSpawnMode,
                                    RootSpawnCall = spawnCall
                                };
                                entry.AddSpawnCall(spawnCall);
                                GeNaEditorUtility.ScheduleSpawn(entry);
                                spawner.SpawnerData.LastSpawnPosition = hitInfo.point;
                            }
                            onRepaint?.Invoke();
                            // Use Event
                            e.Use();
                        ...
                        case EventType.MouseDrag:
                            ...
                            if (raycastHit)
                            {
                                Vector3 direction = hitInfo.point - spawner.SpawnerData.LastSpawnPosition;
                                if (direction.magnitude > spawner.SpawnerData.FlowRate)
                                {
                                    // Paint Mode
                                    if (spawner.SpawnerData.SpawnMode == Constants.SpawnMode.Paint)
                                    {
                                        SpawnCall spawnCall = GeNaSpawnerInternal.GenerateSpawnCall(spawner.SpawnerData, hitInfo);
                                        SpawnerEntry entry = new SpawnerEntry(spawner)
                                        {
            ...
            switch (e.rawType)
            {
                case EventType.MouseUp:
                    mouseUp = true;
                    spawner.SpawnerData.VisualizationFixed = false;
                    break;
            }
            if (mouseUp)
            {
                bool paintedSpawn = spawner.SpawnerData.SpawnMode == Constants.SpawnMode.Paint;
                if (paintedSpawn)
                {
            ...
            var cancel = true;
            var geNaManager = GeNaGlobalReferences.GeNaManagerInstance;
            if (geNaManager != null)
            ...
            if (e.type == EventType.Repaint)
                GeNaSpawnerInternal.DrawVisualization(spawner.SpawnerData);
            spawner.SpawnerData.VisualizationActive = false;
            ...
            if (e.shift)
            {
                if (m_showVisualizer)
                    ...
                    spawner.SpawnerData.VisualizationActive = true;
                        ...
                        switch (e.type)
                        {
                            case EventType.MouseDown:
                            {
                                ...
                                if (raycastHit)
                                {
                                    ...
                                    GeNaSpawnerInternal.SetSpawnOrigin(spawner.SpawnerData, spawnCall, true);
                                    //Let's update ranges - including child spawners.
                                    GeNaSpawnerInternal.UpdateTargetSpawnerRanges(spawner.SpawnerData, hitInfo, true);
                                    GeNaSpawnerInternal.UpdateVisualization(spawner.SpawnerData);
                                    onRepaint?.Invoke();
                                    refreshAabbManager = true;
                                }
                            ...
                            {
                                CreateTempSpawnerData(spawner);
                                spawner.SpawnerData.VisualizationFixed = true;
                                Quaternion rot = Quaternion.LookRotation(hitInfo.point - spawner.SpawnerData.SpawnOriginLocation);
            ...
            {
                spawner.SpawnerData.LoadAabbManager(spawner.SpawnerData.SpawnOriginLocation);
                GeNaSpawnerInternal.UpdateVisualization(spawner.SpawnerData);
            }
            ...
            if (e.shift)
            {
                Handles.color = new Color(0f, 0f, 1.0f, 0.30f);
                foreach (var spawnCall in spawnerData.LastSpawnCalls)
                {
                    Handles.DrawSolidArc(spawnCall.Location, Vector3.up, Vector3.forward, angle, radius);
                }
                if (spawnerData.HasActivePhysicsProtosRecursive())
                {
                    var physicsSettings = spawnerData.PhysicsSettings;
                    var spawnOriginLocation = spawnerData.SpawnOriginLocation;
                    var location = new Vector3(spawnOriginLocation.x, physicsSettings.MinHeightY, spawnOriginLocation.z);
                    Handles.color = new Color(1.0f, 0.0f, 0.0f, 0.30f);
                    Handles.zTest = CompareFunction.LessEqual;
                    Handles.DrawSolidDisc(location, Vector3.up, spawnerData.SpawnRange);
            ...
            {
                if (HasTrees(spawner.SpawnerData.SpawnPrototypes) || HasPrefabs(spawner.SpawnerData.SpawnPrototypes) || spawnCriteria.CheckMaskType == Constants.MaskType.Image)
                {
                    Handles.color = new Color(0f, 0f, 255f, 0.85f);
                    Vector3 position = spawner.SpawnerData.SpawnOriginLocation;
                    if (GeNaUtility.ApproximatelyEqual(placementCriteria.MinRotationY, placementCriteria.MaxRotationY))
                    {
                        ...
                        float size = Mathf.Clamp(spawner.SpawnerData.SpawnRange * 0.2f, 0.25f, 40f);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                // Spawn Algorithm
                if (m_placementCriteria.SpawnAlgorithm == Constants.LocationAlgorithm.Every && spawnerData.ThrowDistance < 0.5f)
        ...
        public static void Serialize(GeNaSpawner spawner)
        {
            ...
            {
                CreateTempSpawnerData(spawner);
                // spawner.SerializeToFile(spawnerData);
            ...
            {
                // var spawnerData = GeNaEditorUtility.GetMainSpawnerData(spawner);
                // spawner.SerializeToFile(spawnerData);
                GeNaSpawnerData spawnerData = spawner.SpawnerData;
                EditorUtility.SetDirty(spawnerData);
        ...
        private void AdvancedPanel(bool helpEnabled)
        {
            EditorGUI.BeginChangeCheck();
            {
                ...
                m_settings.MaxVisualizationDimensions = m_editorUtils.IntSlider("Visualiser Resolution", m_settings.MaxVisualizationDimensions, 1, 512, helpEnabled);
                // Advanced
        ...
        #region Decorators
        [NonSerialized]
        protected readonly Dictionary<IDecorator, Editor> m_decoratorEditors = new Dictionary<IDecorator, Editor>();
        public Editor GetEditor(IDecorator decorator)
        {
            ...
            Editor editor = CreateEditor(decorator as Component) as Editor;
            if (editor is GeNaEditor geNaEditor)
                geNaEditor.SetSpawner(m_spawner);
            m_decoratorEditors.Add(decorator, editor);
            return editor;
        }
        private void EditDecorators(Resource res)
        {
            var decorators = res.Decorators;
