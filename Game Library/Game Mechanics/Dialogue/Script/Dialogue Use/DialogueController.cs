...
namespace KasperDev.Dialogue.Example.Ex01
{
    public class DialogueController : DialogueGetData
    {
        ...
        public bool isHaveChoice = false;
        ...
        // Dalam DialogueController
        private void TypeText(string text, UnityAction onTextComplete)
        {
            dialogueController.currentDialogueText = text; // Simpan teks dialog saat ini
            dialogueController.SetText(text, onTextComplete); // Ganti dengan metode baru untuk mengatur teks di UI
        ...
        private void DialogueToDo()
        {
            ...
            for (int i = currentIndex; i < baseContainers.Count; i++)
            {
                ...
                if (baseContainers[i] is DialogueData_Text)
                {
                    ...
                    TypeText(
                        tmp.Text.Find(text => text.LanguageType == LanguageController.Instance.Language).LanguageGenericType,
                        () => Buttons() // Memanggil metode Buttons() setelah teks selesai
                    );
